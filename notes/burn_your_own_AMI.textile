...
for foo in hadoop-0.20-{tasktracker,datanode,namenode,jobtracker,secondarynamenode} hadoop-{zookeeper-server,hbase-master,hbase-regionserver} cassandra elasticsearch ; do sudo update-rc.d -f $foo remove ; done
...
<pre><code>
  scp -i ~/.hadoop-ec2/keypairs/bonobo.pem ~/.hadoop-ec2/{certs/cert.pem,certs/pk.pem,keypairs/bonobo.pem} ubuntu@50.16.13.146:/tmp
</code></pre>
