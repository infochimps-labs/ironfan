  ...
  class Knife
    class ClusterKill < Knife
      ...
      def run
        ...
        chef_nodes  = servers.map(&:chef_node).compact
        fog_servers = servers.map(&:fog_server).compact
        chef_nodes  = [] if config[:no_chef]
        fog_servers = [] if config[:no_fog]

        target.display

        if chef_nodes.empty? && fog_servers.empty?
          die( "Nothing to delete.", "Exiting.")
        end
