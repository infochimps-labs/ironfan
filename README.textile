...
sudo rm /etc/hostname /etc/chef/{chef_config.json,client.pem,validation.pem} 
sudo rm /var/lib/cloud/data/scripts/* /var/log/chef/* /etc/sv/chef-client/log/main/* /var/log/*.gz /var/log/hadoop/* /tmp/*
sudo rm -rf  /root/.cache /root/.gem ~ubuntu/.cache ~ubuntu/.gem} /etc/hadoop/conf/core-site.xml
sudo bash -c 'for foo in /var/log/{dmesg,syslog,messages,debug,udev,lastlog,*.log} ; do echo -n > $foo ; done'
...
sudo bash -c 'echo -e "#!/bin/sh\n\n# ec2-mkdir-tmp Create /tmp if missing (nicer to bundle without it).\n\nmkdir -p    /tmp\nchmod 01777 /tmp" > /etc/init.d/ec2-mkdir-tmp'
sudo chmod a+x /etc/init.d/ec2-mkdir-tmp
sudo ln -s /etc/init.d/ec2-mkdir-tmp /etc/rcS.d/S36ec2-mkdir-tmp
...
AMI_EXCLUDES=/ebs1,/ebs2,/mnt,/data,/root/.ssh/authorized_keys,/home/ubuntu/.ssh/authorized_keys
