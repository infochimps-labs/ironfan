...
<pre><code>
  # Make one directory and dump all your poolparty and chef stuff in it.
  mkdir ~/.chef
  ln -s ~/.chef ~/.poolparty
  cd hadoop_cluster_chef                                    # wherever that is
  cp ./config/knife.rb ~/.chef                              # edit it, or run 'knife configure -i'
  cp ./config/poolparty-example.yaml ~/.chef/poolparty.yaml # edit it
  ln -s ~/.chef/poolparty.yaml ~/.chef/aws
  # We'll use some opscode cookbooks from a neighbor dir of hadoop_cluster_chef:
  git clone git://github.com/opscode/cookbooks.git ../opscode_cookbooks
</code></pre>
