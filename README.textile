...
<pre>
mkdir -p $DOT_CHEF_DIR/plugins $DOT_CHEF_DIR/bootstrap
cd $DOT_CHEF_DIR/plugins
ln -s $CHEF_REPO_DIR/cluster_chef/lib/cluster_chef/knife/*.rb .
...
<pre>
mkdir -p $CHEF_REPO_DIR/site-cookbooks $CHEF_REPO_DIR/clusters
cd $CHEF_REPO_DIR
cp cluster_chef/clusters/demoweb ./clusters
cd site-cookbooks
ln -s ../cluster_chef/
</pre>
...
# Log in to the "AWS console":http://bit.ly/awsconsole and create a new keypair named @demoweb@. Your browser will download the private key file.
# Move the private key file you just downloaded to your .chef dir, and make it private key unsnoopable, or ssh will complain:

<pre>
mv ~/downloads/demoweb.pem $DOT_CHEF_DIR/demoweb.pem
...
<pre>
    knife cluster launch demoweb master --bootstrap
</pre>
...
<pre>
    knife cluster launch demoweb worker --bootstrap
</pre>
