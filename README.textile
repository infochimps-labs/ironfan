...
If the webui doesn't log you in, try doing @sudo service chef-server-webui restart@ -- it occasionally will fail to create the admin user for some reason.
...
Put the maintain_hadoop_cluster::format_namenode recipe in below hadoop and before hadoop_master.
...
scp -i ~/.hadoop-ec2/keypairs/westchef.pem ~/.hadoop-ec2/{aws_private_setup.sh,certs/cert.pem,certs/pk.pem,keypairs/westchef.pem} ubuntu@ec2-204-236-189-83.us-west-1.compute.amazonaws.com:/tmp
...
ami_bucket=westchef-client-ami-32bit-`date "+%Y%m%d"`
AMI_EXCLUDES=/ebs1,/ebs2,/root/.ssh,/mnt
...
time ( PATH=/usr/bin:$PATH ; ec2-upload-bundle --location $AWS_REGION -b $ami_bucket -m /mnt/$ami_bucket/image.manifest.xml -a $AWS_ACCESS_KEY_ID  -s $AWS_SECRET_ACCESS_KEY ) ;
