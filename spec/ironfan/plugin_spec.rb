...
describe Ironfan::Dsl::Component do
  ...
  context 'when announcing' do
    ...
    it 'configures the correct security groups during discovery' do
      foo_group = Ironfan.realm(:wap).cluster(:foo).cloud(:ec2).security_group('foo')
      bar_group = Ironfan.realm(:wap).cluster(:bar).cloud(:ec2).security_group('bar')
    ...
    it 'configures the correct security groups during bidirectional discovery' do
      baz_group = Ironfan.realm(:wap).cluster(:baz).cloud(:ec2).security_group('baz')
      bif_group = Ironfan.realm(:wap).cluster(:bif).cloud(:ec2).security_group('bif')
    ...
    it 'correctly sets the server cluster even when the client and server facets differ' do
      bam_wak_group = Ironfan.realm(:wap).cluster(:bam).facet(:wak).cloud(:ec2).security_group('bam-wak')
